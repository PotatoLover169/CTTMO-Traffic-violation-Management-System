...
# Get domains from environment variable (comma-separated list)
RECAPTCHA_ALLOWED_DOMAINS = []
domains_str = os.environ.get('RECAPTCHA_ALLOWED_DOMAINS', '')
if domains_str:
    # Split by comma and remove any whitespace
    RECAPTCHA_ALLOWED_DOMAINS = [domain.strip() for domain in domains_str.split(',') if domain.strip()]

# Include all ALLOWED_HOSTS that aren't wildcards
for host in ALLOWED_HOSTS:
    if host != '*' and host not in RECAPTCHA_ALLOWED_DOMAINS and not host.startswith('^') and not host.startswith('*'):
        RECAPTCHA_ALLOWED_DOMAINS.append(host)

# Include 'localhost' and '127.0.0.1' for development
if 'localhost' not in RECAPTCHA_ALLOWED_DOMAINS:
    RECAPTCHA_ALLOWED_DOMAINS.append('localhost')
if '127.0.0.1' not in RECAPTCHA_ALLOWED_DOMAINS:
    RECAPTCHA_ALLOWED_DOMAINS.append('127.0.0.1')